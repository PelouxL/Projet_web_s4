    ...
    function affiche_ami($pdo){
        ...
        if($amis){
          foreach($amis as $ami){
            ...
            echo "<form action='php/discussion.php' method='post'>";
              echo "<button type='submit' name='id' value='" . htmlspecialchars($ami['ami_id']) . "'> D </button><br> ";
            echo "</form>";
              echo "<form action='php/profile_user.php' method='post'>";
...
function retirer_ami($id_ami, $pdo) {
  ...
  if ($stmt->execute()) {
      echo "L'ami a été retiré avec succès.";
  } else {
...
function retirer_demande($id_ami, $pdo) {
  ...
  if ($stmt->execute()) {
      echo "La demande d'amis a été retirée avec succès.";
  } else {
      echo "Erreur lors de la suppression de la demande d'amis.";
  }
}
